...
**WU-4 completed (2026-09-09, pre-apply `971b64c6` SHA `5cf77f7d...`, commit `66738865`):** migrated `RegistryDurableSpineTest` to the central fixture. 5 tests preserved EXACTLY (DREG-1..5: names, count, body, assertions, runIds, seeded fingerprints). The local `coordinator(registry, clock, journal)` factory now delegates to `CoordinatorFixture.default(clock, journal, InMemoryEventStore(), registry)` via the registry-aware overload. Local `noOpCredentialScopePort()` and the `CanonicalDurableRunCoordinator(...)` direct construction deleted; unused imports dropped (`CanonicalNodeDispatcher`, `DefaultEffectReplayPolicy`, `InMemoryReplayCursorStore`, `CredentialScopePort`, `CredentialScopeOutcome`, `CredentialScopeFailure`). Each test creates its own `InMemoryEventStore` per-test (no assert accesses the event sink), matching the pattern of other migrated tests. No seam dependency escapes the fixture surface — the only exposed seam is the custom `StepRegistry` injection, which the registry-aware overload covers directly. Nothing was left explicit in this test. Diff `+3/-22` (19 net removal), only `RegistryDurableSpineTest.kt` touched. Verification (fresh canary, NO WU-2/WU-3 re-run): L0 `:pipeline-application:compileTestKotlin` PASS 2s incremental; L1 `--rerun-tasks` `RegistryDurableSpineTest` PASS 22s, fresh XML 5 tests / 0 failures / 0 errors, timestamp `2026-09-09T06:38:22.824Z`, XML SHA-256 `49ff9d1e096740ed5f0b8bceec4a8faec1f5d72950fd8b985e03c822c8f554ee`, log SHA-256 `1759e4a7e3cfe22e4b3f68943ab86548eef18b57e6b1cf7270fa7c2fd61246df`; guardrail `:pipeline-architecture-tests:test` UP-TO-DATE (incremental; last green baseline preserved). Coordinator composition authority remains `SeamedExecutionRouter` via `buildDefaultExecutionBoundary`; no second router introduced.

**WU-5 completed (2026-09-09, pre-apply `66738865` SHA `49ff9d1e...`, commit `6fc05a8c`):** migrated `CanonicalCoordinatorScopeStackTest` (UAT-LFC1-008-SCOPE-STACK, catchError/warnError scope-stack semantics) to the central fixture. 5 tests preserved EXACTLY (names, count, bodies, assertions, runIds). `makeCoordinator()` now delegates to `CoordinatorFixture.default(clock, journal, InMemoryEventStore())` — the fixture's existing `(clock, journal, eventSink)` signature with default `recorder=null`. No custom overload introduced. Local `noOpCredentialScopePort()` stub and the `CanonicalDurableRunCoordinator(...)` direct construction deleted; unused imports dropped (`CanonicalCoreStepCommand`, `StepOutcome`, `InMemoryReplayCursorStore`, `DefaultEffectReplayPolicy`, `assertInstanceOf`, `TimeUnit`, `CredentialScopePort/Outcome/Failure`, `CanonicalNodeDispatcher` indirectly via the fixture). `SystemClock` retained because the fixture call sites use it as the `clock` arg. Diff `+8/-32` (24 net removal), only `CanonicalCoordinatorScopeStackTest.kt` touched. No seam dependency escapes the fixture surface (no `controlDirRoot`, no `shOptions`, no custom dispatcher, no non-core registry). Verification (fresh canary, NO WU-2/WU-3/WU-4 re-run): L0 `:pipeline-application:compileTestKotlin` PASS incremental UP-TO-DATE; L1 `--rerun-tasks` `CanonicalCoordinatorScopeStackTest` PASS 21s, fresh XML 5 tests / 0 failures / 0 errors, timestamp 2026-09-09T06:45:38.133Z, XML SHA-256 `9cbed2b791b3acc97e4b916306986871a5219c5e1c33544404ba10e874e18ba2`, log SHA-256 `af937827bcbd513e29112bd4cef04868d2e3d9eb610ea10d7373c37cf0eefb1f`; guardarraíl `:pipeline-architecture-tests:test` UP-TO-DATE (last green baseline preserved). Coordinator composition authority remains `SeamedExecutionRouter` via `buildDefaultExecutionBoundary`; no second router introduced. All five canonical-spine coordinator test classes in S2.5.3 are now on the fixture.

**S2.5.3 CoordinatorFixture migration — COMPLETE (2026-09-09).** All five canonical-spine coordinator test classes are on the central fixture: `DurableProtocolInvocationCharacterizationTest` (WU-2, 8 tests, dual + positive), `EchoDurableSpineTest` (WU-3, 3 tests, dual + positive), `RegistryDurableSpineTest` (WU-4, 5 tests, custom-registry overload), `CanonicalCoordinatorScopeStackTest` (WU-5, 5 tests, positive). WU-1 added `CoordinatorFixture` + its own test (3 tests). Single `SeamedExecutionRouter` production routing authority preserved (no second router introduced). Single authority of routing remains `buildDefaultExecutionBoundary`; the recorder is a decorator, never a dispatcher. Hard-gate before LEGACY_REMOVED partly satisfied: tests are now driven through registry/resolver, except the explicit dual-consumer tests in `DurableProtocolInvocationCharacterizationTest` (a1-4) and `EchoDurableSpineTest`.

**S2.5 round gate — INCREMENTAL CHECK (2026-09-09, working tree at `2bef4d7b`):** ran `./gradlew -p v2 check` (incremental, default). The log captured 11 `FAILED` lines in `:pipeline-scripting-kotlin24:test` (7 failures: `ScriptTextEscaperTest` × 3 SCR-N2 + `WithCredentialsCompileIntegrationTest` × 4 IT-001/002/003/006) and 4 in `:pipeline-application:test` (`CompatibilityCorpusTest.fixture14CredentialsBindings`, `UatCompat001CorpusSmokeRunTest`, `UatDsl005TimeoutGrammarTest` × 2). Full worktree evidence (rule V2 #16): rebuilt `c3aaab8a` in `/tmp/base-c3aa` and re-ran the exact same tests — **the same 11 failures reproduce at base** (`/tmp/base-c3aa-app.log` shows the 4 application tests, `/tmp/base-c3aa-script.log` shows the 7 scripting-kotlin24 tests; both `BUILD FAILED` with identical assertion sites). Conclusion: **0 regressions introduced by the S2.5.3 slice.** Pre-existing failures classified: scripting-kotlin24 family (already noted in prior receipts — `ScriptTextEscaperTest` failures dated 2026-09-08, `WithCredentialsCompileIntegrationTest` belongs to the EM-7/credentials scope and predates this pass) and the application test failures (corpus + grammar + checkout-git, all unchanged by fixture migration). The S2.5.3 slice is GREEN under the no-regression rule. No SEAM finding (fresh=1/reuse=0/divergence=0/rejected=0 still hold under registry via the migrated recorder).

**S2.5.7 WU-1 completed (2026-09-09, pre-apply `49111c88`, commit `9024e46e`):** introduced the pure foundation primitives for the spine — `StepAdmissionLaws` value-object (object with `enum ReconciliationResolution` + sealed `AdmissionReport` 7-variant ADT + pure `observe(resolution, executorCalls)`) and `StepAdmissionObserved` `DomainEvent` variant (5 standard + 5 specific fields). Spine NOT touched: `CoordinatorFixture.kt`, `Main.kt`, `CanonicalDurableRunCoordinator.kt`, `CommonExecutionBoundary.kt`, `SeamedExecutionRouterTest.kt`, `DurableProtocolInvocationCharacterizationTest.kt` all unmodified. Required compile-only follow-ons (same pattern as EM-7 `CredentialCleanupFailed`): added the new variant to the non-exhaustive `when` branches of `JsonEventLog.kt`, `InMemoryEventStore.kt`, `SqliteEventStore.kt`, plus updated the sealed-hierarchy count assertions in `DomainEventRoundTripTest.kt` (43 → 44) and `FArchL7DomainEventExhaustivityTest.kt` (43 → 44). Verification (fresh canaries, no S2.5.3 WUs re-run): L0 `:pipeline-application:compileTestKotlin` + `:pipeline-events:compileKotlin` PASS; L1 `StepAdmissionLawsTest` `--rerun-tasks` PASS, fresh XML 7 tests / 0 failures / 0 errors, latest canary timestamp 2026-09-09T07:59:45.296Z (post-commit clean-tree re-run), XML SHA-256 `16359fa095b77b0217350c077370c8279ef9212fff0a7f2ce73cdce557a1e935`, log SHA-256 `b47857b5e4f4ae1b1cb7d8445a41f3323f34723c85dba138848aa04ad4ec4f62` (earlier agent-run canary `93bcc7b00214823da3bd7b7d42984f8463c77391e03f42bc78426e514a9e833d` superseded by the post-commit re-run); L4 `:pipeline-architecture-tests:test` UP-TO-DATE (incremental; last green baseline preserved); `:pipeline-events:test --rerun-tasks` PASS (DomainEventRoundTripTest 14/0/0, JsonEventLogRoundTripTest 28/0/0, InMemoryEventStoreTest 3/0/0, SqliteEventStoreRoundTripTest 1/0/0). Diff stat `+189 / -9` (8 files: 2 NEW + 6 MODIFIED; new files 137 LOC, modified net +52). The 5 follow-on modifications are mandatory production-compile closures, not scope creep.

**S2.5.7 WU-2 completed (2026-09-09, pre-apply `9024e46e`, commit `19abc508`):** introduced the pure family-routing foundation. `FamilyRouter` (object) + `FamilyRoutingDecision` sealed interface (`data object LegacyOnly` | `data class SeamedRouting(legacy: CommonExecutionBoundary, registry: CommonExecutionBoundary)`) + pure `decide(dispatcher, invocationExecutor, stepRegistry, stepKey = null): FamilyRoutingDecision`. Returns `LegacyOnly` when registry is null OR stepKey is null OR registry does not own the key; returns `SeamedRouting` with both pre-built boundaries only when registry is non-null AND stepKey is non-null AND `registry.contains(stepKey)`. Spine NOT touched: `CoordinatorFixture.kt`, `Main.kt`, `CanonicalDurableRunCoordinator.kt`, `CommonExecutionBoundary.kt`, `StepAdmissionLaws.kt`, `SeamedExecutionRouterTest.kt`, `DurableProtocolInvocationCharacterizationTest.kt`, `DomainEvent.kt`, all stores, `buildDefaultExecutionBoundary` (the factory, NOT this slice, is what will switch from the inline `if` to `when(decide(...))`). Real names discovered vs the plan: `LegacyExecutionAdapter.adapt(CanonicalInvocationExecutor): CommonExecutionBoundary` (NOT the assumed `RegistryExecutionBoundary(registry)` ctor — `RegistryExecutionBoundary.adapt()` is a no-arg factory; the registry reachability is established at prepare-time, not at boundary construction), `StepRegistry.contains(PluginStepId): Boolean` (NOT `owns`; the plan's `owns` does not exist), `InMemoryStepRegistry` (default registry used in the test fixture, registered with `CoreEchoStep.registerInto(registry)` so the test gets a real, durable-spine-eligible definition). Signatures adjusted accordingly: `decide(...)` does NOT take a registry constructor arg for the registry boundary (the registry reachability lives at prepare-time); instead it returns the pre-built `SeamedRouting(legacy, RegistryExecutionBoundary.adapt())` so the factory (WU-3) can pass them straight into `SeamedExecutionRouter.route(legacy, registry)`. Pure function: no `runBlocking`, no I/O, no clock; both boundaries are pre-built by the existing legacy/registry factories. `FamilyRouterTest` (4 tests, 0 failures, 0 errors; L1 22s): `no registry returns LegacyOnly`, `registry present without a step key returns LegacyOnly`, `registry present and stepKey owned by registry returns SeamedRouting with two boundaries` (asserts non-null legacy, non-null registry, and `legacy !== registry` so the factory MUST apply the seamed router), `registry present and stepKey NOT owned by registry returns LegacyOnly`. Verification (fresh canaries): L0 `:pipeline-application:compileTestKotlin` PASS 2s; L1 `--rerun-tasks` PASS 22s, fresh XML 4 tests / 0 failures / 0 errors, timestamp `2026-09-09T08:03:34.642Z`, XML SHA-256 `ab8b30561e99043ae990072416543aa3076f2a000addfe2c8760cc65cd329ba8`, log SHA-256 `00d9741910332b2229e775ac185ef43d02be1ca823d357450e2187f82f76ce51`; L4 `:pipeline-architecture-tests:test` UP-TO-DATE (incremental; last green baseline preserved). Diff stat `+181 / -0` (2 NEW files, 82 + 99 LOC, no production modification outside the new files). The seam named: `FamilyRoutingDecision` is the exhaustive `when` target the future `ExecutionBoundaryFactory.build(...)` (WU-3) will match on, replacing the implicit `if (stepRegistry != null)` inside `buildDefaultExecutionBoundary`. Post-commit clean-tree re-run canary (latest, 2026-09-09T08:07:23.473Z): XML SHA-256 `88d6bf6f967d4b7ad62c2d55436f161613ca43ba9e785bc7404192034d4385c9`, log SHA-256 `3e241f102c644203766352e87e1ec4b0f8dd289bd973a785b5edb7fc88e0ca28` (4/0/0).

**Next:** sddk-verify for the spine-consolidation slice (claim: 6 WUs landed, 27+9 spine tests green at L1, 0 regressions vs base c3aaab8a across 19 pre-existing failures).

**S2.5.7 round gate — INCREMENTAL CHECK (2026-09-09, working tree at `86b41236`):** ran `./gradlew -p v2 check` (incremental, default). Headline: `BUILD FAILED in 18m 24s`, 95 tasks (26 executed, 69 up-to-date). 19 failures captured: 4 in `:pipeline-scripting-kotlin24:test` (3 × `ScriptTextEscaperTest.SCR-N2` block/line/single-quotes comments; 1 module summary), 4 in `:pipeline-application:test` (`CompatibilityCorpusTest.fixture14CredentialsBindings`, `UatCompat001CorpusSmokeRunTest.corpus smoke-runs green and satisfies M2 exit criterion`, 2 × `UatDsl005TimeoutGrammarTest.timeout-retry script`), plus 8 new UAT-Local-with-(Path) failures (`UatLocal005CheckoutGitTest.SC-007`, `UatLocal007SandboxProfileTest.SB-S-008/SB-S-010`, `UatLocal008CredentialsTest.CR-BD-027/UAT-L8-CP-001`, `UatLocal009TopStepsTest.CR-U9-008/011/012`), plus the 4 companion WithCredentialsCompileIntegrationTest IT-001/002/003/006 pre-existing from EM-7/credentials. **Full worktree evidence (rule V2 #16):** rebuilt `c3aaab8a` in a scratch clone (`/tmp/jcode-scratch-s257-base-recheck/base-c3aa/v2`) and re-ran the **8 UAT-Local-with-(Path) tests** `--rerun-tasks` against the base — `BUILD FAILED in 5m 1s` with the **same 8 failures**, byte-identical assertion sites (e.g. `UatLocal005CheckoutGitTest.kt:714` → IllegalStateException; `UatLocal007SandboxProfileTest.kt:545/719` AssertionFailedError; etc.). Therefore all 8 UAT-Local (Path) failures are pre-existing host-dependent flakes that the S2.5 gateway at commit `2bef4d7b` did not surface (its preset was incomplete for those UATs at the time). The remaining 11 failures are the ones already cited in commit `49111c88`'s body (scripting-kotlin24 + the 4 application tests from the corpus/grammar/credentials scope). **Conclusion: 0 regressions introduced by S2.5.7.** Pre-existing baseline now widens to 19 known failures (11 already cited + 8 newly cited): scripting-kotlin24 family (7), WithCredentialsCompileIntegrationTest (4), Compat corpus (1), UAT-Dsl005 grammar (2), UAT-Local with-(Path) family (8). S2.5.7 slice is GREEN under the no-regression rule. Spine NOT touched at the call-site level (still a single production routing authority; the ExecutionBoundaryFactory seam is bit-equivalent to the inline `if (stepRegistry != null)` contract).

**S2.5.7 spine-consolidation slice — COMPLETE (2026-09-09).** 6 WUs landed:
1. `9024e46e` — WU-1: `StepAdmissionLaws` pure observer (7 `AdmissionReport` variants + `ReconciliationResolution` enum) + `StepAdmissionObserved` domain event (44-event sealed-hierarchy exhaustive; 5 stores + 2 tests follow-on).
2. `19abc508` — WU-2: `FamilyRouter` + `FamilyRoutingDecision` sealed ADT (`LegacyOnly` data object | `SeamedRouting(legacy, registry)` data class) + pure `decide(...)` (4 tests).
3. `60663cc8` — WU-3: `ExecutionBoundaryFactory` named producer seam + recorder decorator + `RecordingBoundary` (private wrapper over produced boundary; counter + recorder invocation; never re-routes) (4 tests).
4. `300e061e` — WU-4: forwarder in `CommonExecutionBoundary.kt` (1-line delegation to `ExecutionBoundaryFactory.build`) + **factory binary-policy correction** — the initial key-aware policy (registry.contains + stepKey to seam) had to be replaced with the legacy-bit-equivalent binary rule (`stepRegistry != null → SeamedRouting; otherwise → LegacyOnly`) because the canonical coordinator does not know the step key at boundary-build time. stepKey stays advisory-only on the factory signature; FamilyRouter untouched.
5. `f5549178` — WU-5: coordinator wires `ExecutionBoundaryFactory.build(...)` directly (the `?: buildDefaultExecutionBoundary(...)` call-site in `CanonicalDurableRunCoordinator.kt:322` now calls the factory directly; the forwarder is kept as a source-compatibility shim).
6. `df745d77` — WU-6: `StepAdmissionObservedTest.kt` — 9 tests (7 laws-pure per ADT variant + 2 factory build-shape contract).

Combined L1 coverage across the spine (final round, `--rerun-tasks` on 6 test classes): **27 + 9 = 36 tests / 0 failures / 0 errors** (DurableProtocolInvocationCharacterizationTest 8/0/0, SeamedExecutionRouterTest 2/0/0, ExecutionBoundaryFactoryTest 4/0/0, RegistryDurableSpineTest 5/0/0, EchoDurableSpineTest 3/0/0, CanonicalCoordinatorScopeStackTest 5/0/0, StepAdmissionObservedTest 9/0/0). L4 `:pipeline-architecture-tests:test` UP-TO-DATE every WU. No second dispatcher / second router / second metadata authority introduced. Capabilities used = exactly today's (`EVENT_SINK_CAPABILITY` + fingerprint + replay policy); NO new capabilities added. Per-step observability is the law: `StepAdmissionLaws.observe(resolution, executorCalls)` is the typed event each observer reads.

**S2.5.7 WU-5 completed (2026-09-09, pre-apply `300e061e`, commit `f5549178`):** coordinator now wires `ExecutionBoundaryFactory.build(dispatcher, invocationExecutor, stepRegistry)` directly inside `CanonicalDurableRunCoordinator` (line 322-325; replaced the previous `?: buildDefaultExecutionBoundary(...)` forwarder call-site). The factory is the structural-switch authority (binary legacy-bit-equivalent `if (stepRegistry != null) → SeamedRouting; otherwise → LegacyOnly`, established in WU-4); the forwarder `buildDefaultExecutionBoundary` in `CommonExecutionBoundary.kt` is kept as a source-compatibility shim for external callers (and remains the boundary the test fixture + Main use). Diff stat `+8 / -4` across 1 file (`CanonicalDurableRunCoordinator.kt`). No new imports needed (same package: `dev.rubentxu.pipeline.v2.application.durable`). Spine NOT touched: `CoordinatorFixture.kt`, `Main.kt`, `CommonExecutionBoundary.kt`, `StepAdmissionLaws.kt`, `FamilyRouter.kt`, `ExecutionBoundaryFactory.kt`, `SeamedExecutionRouter.kt`, `buildDefaultExecutionBoundary`, all tests, all stores. Verification (fresh canary, NO earlier canaries re-used, 2026-09-09T08:48:30Z): L0 `:pipeline-application:compileTestKotlin` PASS 2 s; L1 `--rerun-tasks` on 6 test classes covering the spine all-green **27 tests / 0 failures / 0 errors** — `DurableProtocolInvocationCharacterizationTest` 8/0/0, `SeamedExecutionRouterTest` 2/0/0, `ExecutionBoundaryFactoryTest` 4/0/0, `RegistryDurableSpineTest` 5/0/0, `EchoDurableSpineTest` 3/0/0, `CanonicalCoordinatorScopeStackTest` 5/0/0. L1 log SHA-256 `2131819bb030a8aa3068e1eacbbb4bb1d4f2000bd407e40d448a84e8383be641`; L4 `:pipeline-architecture-tests:test` UP-TO-DATE. Post-commit clean-tree HEAD `f5549178` ahead of `origin/main` by 21 commits.

**S2.5.7 WU-6 completed (2026-09-09, pre-apply `f5549178`, commit `df745d77`):** added `StepAdmissionObservedTest.kt` (136 LOC, 9 tests, 0 failures, 0 errors) — characterization of the spine through the laws and the named factory seam. Two suites: (1) **7 laws-pure tests**, one per `AdmissionReport` variant — `Fresh → C1FreshExecutedOnce(executorCalls)`, `ReuseCompleted → C2ReuseNoExecute`, `RejectedDecode → C3`, `Diverged → C4`, `SchemaPrecedence → C5`, `OverlayApplied → C6`, `Other → Other` — each asserts the ADT variant AND its executorCalls field so a regression in either the when-branching or the field wiring fails fast and bit-faithfully; (2) **2 factory build-shape tests** — `build(null recorder, null registry)` returns a non-null boundary; `build(user-supplied recorder, ...)` preserves the recorder reference via identity (the wrapper around the produced boundary is private to the factory, so this suite asserts the contract shape; the counter-precise behaviour is covered by `ExecutionBoundaryFactoryTest` at 4/0/0). No production modification. Spine NOT touched: CoordinatorFixture, Main, CanonicalDurableRunCoordinator, CommonExecutionBoundary, StepAdmissionLaws, FamilyRouter, ExecutionBoundaryFactory, SeamedExecutionRouter, buildDefaultExecutionBoundary, all stores. Diff stat `+136 / 0` (NEW file). Verification (fresh canary, NO earlier canaries re-used, 2026-09-09T08:50:49Z): L0 `:pipeline-application:compileTestKotlin` PASS 9 s; L1 `--rerun-tasks` `StepAdmissionObservedTest` PASS 21 s, fresh XML **9 tests / 0 failures / 0 errors**, timestamp `2026-09-09T08:50:49.370Z`, XML SHA-256 `9c3f998aaf600a5e4f8ec66f2237898f255a903c3577302848bb238c2a727df1`, log SHA-256 `d5f7ad0fdc96a23e86ea82854e12a16e5da9464ff58b29429ae1696c1c7e2258`; L4 `:pipeline-architecture-tests:test` UP-TO-DATE. Post-commit clean-tree HEAD `df745d77` ahead of `origin/main` by 22 commits. S2.5.7 spine-consolidation slice COMPLETE — all 6 WUs landed (`9024e46e`, `19abc508`, `60663cc8`, `300e061e`, `f5549178`, `df745d77`).

**S2.5.7 WU-4 completed (2026-09-09, pre-apply `60663cc8` SHA `9bfa214f...`, commit `300e061e`):** coordinated two changes that preserve bit-a-bit behaviour of the original inline `if (stepRegistry != null)` branch in `buildDefaultExecutionBoundary`. (1) `CommonExecutionBoundary.kt`: the free function `buildDefaultExecutionBoundary(dispatcher, invocationExecutor, stepRegistry)` is now a one-line forwarder over `ExecutionBoundaryFactory.build(dispatcher, invocationExecutor, stepRegistry)`. Signature + KDoc preserved (added note: "As of S2.5.7 this function is retained for source compatibility as a forwarder over `ExecutionBoundaryFactory.build`; the structural decision lives in `ExecutionBoundaryFactory` / `FamilyRouter`. Removal is future cleanup after legacy adapter retirement."). The two call-sites — `CanonicalDurableRunCoordinator` and `DurableProtocolInvocationCharacterizationTest` (CoordinatorFixture path) — compile unchanged. (2) `ExecutionBoundaryFactory.kt`: factory now applies the binary legacy-bit-equivalent rule — `stepRegistry != null → SeamedRouting` (over pre-built legacy + `RegistryExecutionBoundary.adapt()` via `SeamedExecutionRouter.route`), regardless of `stepKey` (which stays advisory-only on the signature); `stepRegistry == null → LegacyOnly`. FamilyRouter is untouched and the finer-grained `stepKey`+`registry.contains` refinement there remains as the optional per-step refinement for a future per-step entry point. The canonical coordinator does not know the step key at boundary-build time (registry reachability is decided at prepare-time inside the registry boundary), so the factory short-circuits the binary case directly. Diagnostic note: the initial key-aware factory policy (stepKey != null AND registry.contains to seam) caused 5 failures across the frozen spine characterization tests — `C1 fresh execution invokes the effective executor exactly once` (DurableProtocolInvocationCharacterizationTest:92), `flipped echo runs via the registry family emitting EchoOutputCaptured` (EchoDurableSpineTest:73), `scope leak across stages throws IllegalStateException` + `happy path echo pipeline succeeds` (CanonicalCoordinatorScopeStackTest:157 / 180), `DREG-1 fresh registry valid executes codec and handler once on the durable spine` (RegistryDurableSpineTest:170). All 5 fail with `EngineInvariantViolation: LegacyExecutionAdapter received a non-legacy PreparedExecution; it cannot route it` — the registry-prepared execution was reaching the legacy-only adapter because `FamilyRouter.decide` returned `LegacyOnly` when `stepKey == null`. The binary factory rule restores the legacy inline-`if` contract without touching FamilyRouter or any consumer. Diff stat `+46 / -24` across 2 files (`CommonExecutionBoundary.kt` `+5 / -9`, `ExecutionBoundaryFactory.kt` `+41 / -15`). Spine NOT touched: `CoordinatorFixture.kt`, `Main.kt`, `CanonicalDurableRunCoordinator.kt`, `FamilyRouter.kt`, `StepAdmissionLaws.kt`, `SeamedExecutionRouter.kt`, all stores, all tests. Verification (fresh canary, NO earlier canaries re-used, 2026-09-09T08:46:18Z timestamp on first XML): L0 `:pipeline-application:compileTestKotlin` PASS 2 s; L1 `--rerun-tasks` on 6 test classes covering the spine all-green **27 tests / 0 failures / 0 errors** — `DurableProtocolInvocationCharacterizationTest` 8/0/0 XML SHA-256 `b5cbcf4e18ccbe06f5184ca29eef5ffc77906cff1e74a03a37567d737da1521f`, `SeamedExecutionRouterTest` 2/0/0, `ExecutionBoundaryFactoryTest` 4/0/0, `RegistryDurableSpineTest` 5/0/0, `EchoDurableSpineTest` 3/0/0, `CanonicalCoordinatorScopeStackTest` 5/0/0. L1 log SHA-256 `8edc11e558595425e81099f5590acb8bec428fa6c8cfa90d87f82740c59b8d85` (recorded at 2026-09-09T08:46:14Z after the binary-policy correction); L4 `:pipeline-architecture-tests:test` UP-TO-DATE (incremental; last green baseline preserved). Post-commit clean-tree HEAD `300e061e` ahead of `origin/main` by 19 commits.

**S2.5.7 WU-3 completed (2026-09-09, pre-apply `19abc508` SHA `88d6bf6f...`, commit `60663cc8`):** introduced the named producer seam `object ExecutionBoundaryFactory.build(dispatcher, invocationExecutor, stepRegistry, stepKey = null, recorder: CommonExecutionBoundary? = null): CommonExecutionBoundary`. Calls `FamilyRouter.decide(...)` and exhaustively `when`-matches the returned `FamilyRoutingDecision`: `LegacyOnly` -> `LegacyExecutionAdapter.adapt(...)` built in the factory (with the same invocationExecutor/dispatcher fallback the router would use); `SeamedRouting` -> `SeamedExecutionRouter.route(decision.legacy, decision.registry)`. The `when` is exhaustive: adding a new `FamilyRoutingDecision` variant breaks the compile by design. When `recorder != null`, the produced boundary is wrapped in a private `RecordingBoundary` class that increments its own call counter, invokes the user-supplied recorder once per call (so test-side assertions on the recorder's counter observe the call), delegates the actual execution to the produced boundary, and returns the produced boundary's `StepOutcome` unchanged. The recorder NEVER re-implements family routing; the productive `SeamedExecutionRouter` authority remains in charge. The free function `buildDefaultExecutionBoundary` is untouched (WU-4 will turn it into a forwarder). Spine NOT touched: `CoordinatorFixture.kt`, `Main.kt`, `CanonicalDurableRunCoordinator.kt`, `CommonExecutionBoundary.kt`, `StepAdmissionLaws.kt`, `FamilyRouter.kt` (and its test), `SeamedExecutionRouter.kt`, `buildDefaultExecutionBoundary`, all stores. Diff stat `+312 / 0` (2 NEW files: `ExecutionBoundaryFactory.kt` 108 LOC, `ExecutionBoundaryFactoryTest.kt` 204 LOC). Real names discovered vs the plan: (1) `LegacyOnly` is a `data object` (NOT `data class LegacyOnly(val legacyBoundary: CommonExecutionBoundary)`) in WU-2 — `decision.legacyBoundary` does not compile. The factory therefore re-builds the legacy boundary for the LegacyOnly case via `LegacyExecutionAdapter.adapt(...)` using the same invocationExecutor/dispatcher fallback. `FamilyRouter` and `FamilyRouterTest` are untouched (preserving the WU-2 surface). (2) The recorder parameter is itself a `CommonExecutionBoundary`; the wrapper invokes it once per call so its counter is observable (`recorder.calls >= 1` in test 3). (3) JUnit 5 quirk: a `@Test` method declared as `= runBlocking { assertThrows(...) }` infers a non-Unit return type and Gradle/JUnit skip the test silently. The fix is to declare the method as `fun ...() { runBlocking { ... } }` (return type Unit) and wrap the throwing call inside `runBlocking { ... }` within `assertThrows`. Test #1 (`LegacyOnly boundary when registry is null`) was rewritten accordingly; originally it had the failing pattern. (4) The 4 tests in the canary XML are: legacy payload succeeds + registry payload throws on LegacyOnly; legacy payload succeeds + non-Failure on SeamedRouting (proves not the legacy boundary alone); recorder wrapper increments the user-supplied recorder's counter; null recorder unwrapped (registry payload throws). Verification (fresh canary, NO WU-1/WU-2 re-run): L0 `:pipeline-application:compileTestKotlin` PASS 3 s; L1 `--rerun-tasks` `ExecutionBoundaryFactoryTest` PASS 21 s, fresh XML **4 tests / 0 failures / 0 errors**, timestamp `2026-09-09T08:25:33.547Z`, XML SHA-256 `71972da4e6b7cffd2fcbb050bfabbf303d145fe8a1e64d17fbe8c66dbb82129b`, L1 log SHA-256 `54e0534266c879efce083d5be4d13fd51dcbf825f4b08ccfb4bc6aa9e66f88c0`; L4 `:pipeline-architecture-tests:test` UP-TO-DATE (incremental; last green baseline preserved). Coordinator composition authority remains `SeamedExecutionRouter` via `buildDefaultExecutionBoundary`; no second router introduced. Post-commit clean-tree re-run canary (latest, 2026-09-09T08:30:00.953Z): XML SHA-256 `9bfa214f2dffa57178eb9cec1d5504c20f50ae6fca590ab1ffee84863790f533`, log SHA-256 `fc5769218505501378b7415169be6335f71593cf39a531694346f17fd768b834` (4/0/0).
...
# GATE EVIDENCE — sddk-verify on S2.5.7 spine-consolidation slice (HEAD fa7d91b0, 2026-09-09)
...
**G3-A3 completed (2026-09-09, pre-apply `53d096b2`, TBD commit):** widens `CommonExecutionBoundary.execute()` to return `CommonExecutionResult(outcome, encodedOutput?)` atomically (Path (a)). Renamed `RegistryExecutionResult` -> `CommonExecutionResult` at the seam. Three coordinator sites (`Execute`, `RecoverRunning`, `RejectedAbort`) consume `CommonExecutionResult` and project `encodedOutput` into `OperationOutput(result, durationMs, finishedAt)`. Legacy paths lift to `CommonExecutionResult(outcome, encodedOutput = null)`. 8 existing test classes mechanically migrated to consume `.outcome`; 1 new test class (`A3DurableProjectionCharacterizationTest`, 3 tests) added. Diff stat `+~520 / -~85`. Spine NOT touched: `CoordinatorFixture`, `Main`, `SeamedExecutionRouter`, `ExecutionBoundaryFactory`, `RecordingBoundary`, `SeamedExecutionRouterTest`, `StepAdmissionLaws`, `FamilyRouter`, `DomainEvent`, all stores. Verification (fresh canary, no S2.5.7 canaries re-used): L0 `:pipeline-application:compileKotlin` + `compileTestKotlin` PASS 14s + 17s; L1 `--rerun-tasks` on 12 test classes (`EchoStepContractSuiteTest`, `ReconciliationStatusOnlyTest`, `ReplayOutputDecouplingTest`, `StepAdmissionObservedTest`, `CoordinatorFixtureTest`, `StepExecutionBoundaryTest`, `SeamedExecutionRouterTest`, `ExecutionBoundaryFactoryTest`, `GenericRegistryExecutionCarrierTest`, `RegistryExecutionOutcomeTest`, `StepOutcomeEncodedOutputDistinctionTest`, `A3DurableProjectionCharacterizationTest`) PASS 19s, fresh XML **81 tests / 0 failures / 0 errors** (per-class counts in `docs/v2/07-uat/LB02_G3_A3_ATOMIC_EXECUTION_SEAM.md`). Pre-existing failures NOT introduced by A3 (rule 16 verified via `git stash + test + git stash pop`): `DualExecutionSeamCharacterizationTest` (2), `DurableProtocolInvocationCharacterizationTest` (1), `UatDsl001JenkinsFamiliarityTest` (1+), `UatDsl005TimeoutGrammarTest` (2+), `UatDsl006BodyExecutionTest` (4) — all reproduce at base `53d096b2`. Inert supporting files left uncommitted (still on disk, still quarantined): `ShellOperations.kt`, `ShellOperationsCapabilityKey.kt`, `ShOperationsAdapter.kt`, and the modified `CanonicalRuntimeCapabilityAccess.kt` revert. Per-step observability law preserved (the `Execute`, `RecoverRunning`, `RejectedAbort` paths still emit `StepStarted`/`StepFailed`/`StepFinished` exactly as before; the carrier is the only thing that grew). ReplayPolicy laws unchanged (no change to `MEMOIZED`/`RERUN`/`NEVER`; `RerunOperation.output` populated only when `encodedOutput != null`, which is independent of the policy). Journal schema unchanged (no field added/renamed/removed). Fingerprint unchanged. Coordinator does not know typed O (no codec calls in coordinator; only `EncodedStepValue.value` String read).

**G3-A4.0 grounding completed (2026-09-09, doc only, not committed separately):** `docs/v2/00-context/LB02_G3_A4_0_SH_GROUNDING.md` documents the current-vs-target Sh authority chain and identifies 6 contract gaps. Critical: `RecoveryPolicy` is not on `StepContract`/`StepDescriptor` today, so `RegistryStepMetadataResolver` builds `recoveryPolicy = None` by default → blocks A4.10. Committed in A4.1's atomic commit `85d0b1f1`.

**G3-A4.1 completed (2026-09-09, pre-apply `f4d0ea90`, commit `85d0b1f1`):** `StepDescriptor.recoveryPolicy: RecoveryPolicy = RecoveryPolicy.None` is now the single source of truth for pre-decode recovery semantics. `RegistryStepMetadataResolver.composite` propagates `descriptor.recoveryPolicy` into the resolved `StepMetadata` for non-legacy keys (no per-Step branches). `CoreShellStep.descriptor` declares `recoveryPolicy = RecoveryPolicy.ExternalSubprocess`, mirroring `CanonicalCoreStepMetadata["core.sh"]` byte-for-byte for replayPolicy, recoveryPolicy, and effects. Added `A4_1DescriptorRecoveryCharacterizationTest.kt` (7 tests): default invariance, Echo unchanged, CoreShellStep declaration, registry propagation, legacy parity during dual phase, full pre-decode metadata parity (A4.1.5), and "no per-Step branching" extensibility. Diff stat `+523 / -16` (4 production files + 2 docs + 1 test). Inert quarantined files still untouched: `ShellOperations.kt`, `ShellOperationsCapabilityKey.kt`, `ShOperationsAdapter.kt` (will integrate in A4.2). Verification (fresh canary, no G3-A3 canaries re-used): L0 `:pipeline-application:compileTestKotlin` PASS 21s; L1 `--rerun-tasks` on 4 test classes (`A4_1DescriptorRecoveryCharacterizationTest`, `RegistryStepMetadataResolverTest`, `CoreShellStepTest`, `EchoStepContractSuiteTest`) PASS 13s, fresh XML **37 tests / 0 failures / 0 errors**. Per-class: A4_1DescriptorRecoveryCharacterizationTest 7/0/0 (XML SHA-256 `1ff87a4a2ac6a2687a1a9cfce98e69a7078e84b951b274fbeee57140b57f6534`), RegistryStepMetadataResolverTest 3/0/0 (`096e1b7d6b34054bd37cdc09f5c256103b7365ccb186d0228035fea498f20d24`), CoreShellStepTest 10/0/0 (`fc62908f5ddf8556429c0c392b10080a3a7d2b7cf43c352b671786fa2d283b95`), EchoStepContractSuiteTest 17/0/0 (`fc2c12469bded2abc3f022df77104924a2867744ca3bda6aaf0ab5679968d967`). L2 full `:pipeline-application:test` (480s, incremental): pre-existing failure count did NOT increase — base `f4d0ea90` had 38 unique failures; A4.1 has 36 (the 2-line gap is `SB-S-008` not run with the explicit `--tests` filter; the 36 failures are the same UAT-subprocess family already documented at `53d096b2`). L4 `:pipeline-architecture-tests:test` UP-TO-DATE. Gating facts for A4.10: unchanged. Stop condition "recovery irreducibly depends on `CanonicalCoreStepCommand.Sh`" — NOT MET; the metadata layer is now Step-agnostic. Journal schema, fingerprint, replay policy, coordinator Step-agnosticism — all preserved.

**G3-A4.2 completed (2026-09-09, pre-apply `85d0b1f1`, commits `b5ff181c` + `0607ad72`):** declared `SHELL_OPERATIONS_CAPABILITY` for `core.sh` and proved the capability-routed handler discipline. `CoreShellStep.handler` asks for the typed `ShellOperations` port via `ctx.capabilities.get(SHELL_OPERATIONS_CAPABILITY)`, invokes it exactly once with the typed `command` + execution identity, and projects the typed `ShellInvocationResult` 1:1 into `CoreShellOutput`. The `ShOperationsAdapter` (production source, no longer quarantined) implements the port by delegating to `ShExecution.invokeShell`. The handler NEVER reaches `CanonicalRuntimeContext`, the journal, the durable-shell substrate, or `eventSink`. The quarantined files (`ShellOperations.kt`, `ShellOperationsCapabilityKey.kt`, `ShOperationsAdapter.kt`) become unquarantined production sources.

**Domain extension:** `StepHandler.execute` and `StepInvoker.invoke` become `suspend` (LB-02 / G3-A4.2) so handlers can reach the suspend `ShellOperations.invoke` without `runBlocking`. Pure-function handlers (e.g. `core.echo`) ignore the suspend modifier — the change is a typed-seam extension, not a domain semantics change. No ADR forbids suspend on `StepHandler`.

**A3 gap closed:** `RegistryExecutionBoundaryTest` had two assertions that compared `StepOutcome.Success` directly against the boundary return value; they had been failing since A3 (`CommonExecutionResult` carrier). Both tests now project `outcome.outcome`. A4.2 catches the gap in-scope so L1 evidence is clean.

**Diff stat:** +855/-66 across 12 files (3 NEW production sources + 1 NEW test + 2 MODIFIED docs + 6 MODIFIED production/test). Inert supporting files (none — all quarantined files are now committed). Verification (fresh canary, no A4.1 canaries re-used): L0 `:pipeline-application:compileKotlin` + `:pipeline-application:compileTestKotlin` PASS 14s + 13s; L1 `--rerun-tasks` on 12 test classes (`A4_2ShellOperationsCapabilityTest`, `CoreShellStepTest`, `A4_1DescriptorRecoveryCharacterizationTest`, `RegistryStepMetadataResolverTest`, `CoreEchoSeamTest`, `EchoStepContractSuiteTest`, `RegistryDurableSpineTest`, `RegistryExecutionPreparationTest`, `RegistryExecutionBoundaryTest`, `RegistryExecutionOutcomeTest`, `StepRegistryTest`, `StepOutcomeEncodedOutputDistinctionTest`) PASS 14s, fresh XML **91 tests / 0 failures / 0 errors**. Per-class: A4_2ShellOperationsCapabilityTest 14/0/0 (XML SHA-256 `b756bf1b353bcc6a`), CoreShellStepTest 11/0/0 (`1938577722562884`), A4_1DescriptorRecoveryCharacterizationTest 7/0/0 (`951700a38b69d5b2`), RegistryStepMetadataResolverTest 3/0/0 (`43e44a99ce566e77`), CoreEchoSeamTest 7/0/0 (`0d3663780753b562`), EchoStepContractSuiteTest 17/0/0 (`fc050b093b34a974`), RegistryDurableSpineTest 5/0/0 (`a0c294373a62f28f`), RegistryExecutionPreparationTest 5/0/0 (`f08cef2135a89448`), RegistryExecutionBoundaryTest 6/0/0 (`2c33ba52bcc6edcd`), RegistryExecutionOutcomeTest 4/0/0 (`1fa11bbdb297d368`), StepRegistryTest 8/0/0, StepOutcomeEncodedOutputDistinctionTest 4/0/0 (`00b4b4a2fa48cf69`). L4 `:pipeline-architecture-tests:test` UP-TO-DATE. Pre-existing failure count did NOT increase (the 2 RegistryExecutionBoundaryTest failures closed are themselves a fix; the 36 UAT-subprocess failures documented in A4.1 are unchanged). Coordinator `core.sh` knowledge = 0 (gate verified). Stop condition "recovery irreducibly depends on `CanonicalCoreStepCommand.Sh`" — NOT MET. Docs-only commit `b5ff181c` adds AGENTS.md operational rules: "Pre-decode durable metadata belongs to StepDescriptor (A4.1)" + "Capability-routed handler discipline (A4.2)".

**AGENTS.md delta:** Two rules added under STEP CONSTITUTION § fitness tests; reference implementation named is `core.echo` (CERTIFIED); `core.sh` will become a second reference once CERTIFIED.

## LB-02 / A4 production flip follow-up (2026-09-09)
- Commits: `615dbbdf` docs, `3115963f` implementation, `06f69c21` evidence, followed by `d87a9188` correcting a stale legacy-membership test and `2bd477a8` recording that receipt.
- Production `core.sh` is now registry-primary: generic structural membership selection, one composition-root registration, capability admission through `SHELL_OPERATIONS_CAPABILITY`, and descriptor metadata authority. Legacy Shell command/decoder/dispatcher/metadata are deliberately retained for A5.
- Focused evidence: 14 classes / 154 tests / 0 failures / 0 errors. Installed `04-sh.pipeline.kts` CLI smoke passed with `StepStarted`, exactly one `EchoOutputCaptured("hello from sh\\n")`, `StepFinished`, and `RunFinished(success)`.
- Regression investigation: a full `:pipeline-application:test` attempt hit the fixed 600-second budget. It exposed one A4-owned stale assertion (`CanonicalCoreStepCommandRegistryTest` expected `core.sh` in `LEGACY_PLUGIN_IDS`), corrected in `d87a9188`; direct class rerun passed 15/0/0. The two exposed CLI failures were the documented pre-existing `core.echo` canonical-ID gap also seen in UatCompat001.
- Broad `check` likewise exhausted 600 seconds amid unrelated suite failures. Do not claim full-project green. The scoped A4 route is validated; A5 remains next, with no AGENTS.md update until certification.
