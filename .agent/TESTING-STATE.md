...
**Next:** S2.5.7 spine consolidation (the per-step admission / family-router spine that ties S2.5.2..S2.5.6 together) → S2.5 gate close → then S3.1 remove `CanonicalCoreStepCommand.Echo`, S3.2 remove legacy decode/dispatch, S3.3 remove legacy metadata/catalogue, S3.4 activate `Lfc2RegistryFamilyFitnessTest`, S3.5 build `StepContractSuite` and certify (else record `IMPLEMENTED_UNCERTIFIED`).
