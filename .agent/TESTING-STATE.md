...
**Next:** S2.5.7 WU-5 — coordinator wires `ExecutionBoundaryFactory.build(...)` and observes `StepAdmissionLaws.observe(...)` over the seam's effective executor-call count. Reads `StepAdmissionObserved` events back from the durable event store on the same run.
