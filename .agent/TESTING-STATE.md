...
**Next:** S2.5.6 dual tests transform — migrate the remaining dual-only classes to `CoordinatorFixture.negativeNoRegistry(clock, journal, eventSink)` (currently the only dual consumers in this pass are within the already-migrated `DurableProtocolInvocationCharacterizationTest` and `EchoDurableSpineTest`). If S2.5.6 has no further migrations, proceed to gate S2.5 (apply/verify boundary; round gate `./gradlew -p v2 check` incremental; record pre-existing baseline failures before declaring green).
