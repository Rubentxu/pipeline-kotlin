 ...
 * 
 * This consumer is optimized for the push-based logging system and provides:
 * - Direct onEvent() method calls from DefaultLoggerManager
 ...
 * 
 * Architecture differences from legacy Flow-based approach:
...
class BatchingConsoleConsumer(
    ...
    private val processingScope = CoroutineScope(
        SupervisorJob() + 
        CoroutineName("BatchingConsoleConsumer") + 
        Dispatchers.IO
    )
    ...
    /**
     * Called by DefaultLoggerManager when a log event is available.
     * 
    ...
    override fun onError(event: LogEvent, exception: Throwable) {
        System.err.println("BatchingConsoleConsumer error processing event: ${exception.message}")
    ...
    private fun startBatchProcessing() {
        processingJob = processingScope.launch {
            try {
                ...
                System.err.println("Critical error in BatchingConsoleConsumer: ${e.message}")
     ...
     */
    fun getStats(): BatchingConsumerStats {
    ...
    fun shutdown() {
        ...
        try {
            ...
            System.err.println("Error during BatchingConsoleConsumer shutdown: ${e.message}")
        }
    }
    
    /**
     * Performance statistics for the batching console consumer.
     */
    data class BatchingConsumerStats(
        ...
        fun summary(): String {
            return "BatchingConsumer: ${String.format("%.0f", eventsPerSecond)} events/sec, " +
