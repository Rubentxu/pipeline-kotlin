...
```text
CERTIFIED:                10  (9 core + example.uppercase external reference)
IMPLEMENTED_UNCERTIFIED:  ~9 (8 block steps working + waitUntil registry stub)
LEGACY executable:         4  (cleanWs, load, waitUntil, archiveArtifacts)
...
| E0-E runtime utilities | isUnix CERT; milestone CERT; pwd registry (G7 BLOCKED LFC-2R2); waitUntil/load pending; timestamps/ansiColor NOT_STARTED | 2/7 |
...
```text
Burned:   9/12 (echo, sh, error, sleep, writeFile, emit.event, isUnix, deleteDir, milestone)
Borrado físico legacy: los 9 + pwd (G5)
Registry-but-uncertified: pwd (LFC-2R2), waitUntil (stub)
Legacy vivo (4): cleanWs, load, waitUntil, archiveArtifacts
Preparación local sin push: cleanWs G1-G3 (bdd4f81e), archiveArtifacts G0+G1 (291cb98d)
```

## Orden de ataque acordado

1. cleanWs G4→G8 (gemelo de deleteDir)
2. archiveArtifacts G2→G8 (3 deltas G2 + 7 pins S3 rojos pre-existentes documentados)
3. waitUntil: desbloqueo vía ADR-0081 BodyInvoker (draft local @ e2f24f04)
4. load: según SPIKE-018 (boundary ScriptLoader/PipelineCompiler)
