...
**Authority:** ADR-0065 (accepted 2026-09-05)
...
### Incremental disposition (2026-09-06)

The active `PipelineRun` shell paths now consume `ShellInvocationResult`
directly: linear `sh`, parallel-branch `sh`, and `withEnv` nested branch
`sh`. The lifecycle walker performs one exhaustive interpretation at its
outer execution seam so `Interrupted(TIMEOUT)` remains `timeout`, other
interruptions remain failures, and `Failed` retains its failure kind for the
`StepFailed` event. `PipelineRun` remains product-live and is not deprecated.

`ShExecution.runShellCommand(...): String` remains the single explicitly
deprecated compatibility adapter; it has no active production caller. This is
not deletion evidence: compatibility fixtures and the EM-10 full-gate exit
criteria still govern its removal.
