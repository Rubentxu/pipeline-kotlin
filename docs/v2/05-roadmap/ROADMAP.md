...
**Bloque de distribución por demanda, después de RP-5 (WU-RP-060..065, PLANNED / NOT_STARTED).** Ampliar WU-RP-043 y la distribución aprobada en ADR-0089 SIN crear otro motor ni otro lenguaje de pipeline. Autoridad detallada de diseño/aceptación: [MULTICHANNEL_RELEASE_DOGFOOD_PLAN.md](MULTICHANNEL_RELEASE_DOGFOOD_PLAN.md) y [UAT_RELEASE_CHANNELS.md](../07-uat/UAT_RELEASE_CHANNELS.md). Ninguna de estas WUs puede empezar implementación pública antes del cierre verificable de RP-5; la preparación documental o spikes sin cambios públicos no cambian el NEXT_WU de SESSION_POINTER (actualmente WU-RP-046). Los números se reservan aquí y se reconcilian antes de activar cada WU.

- **WU-RP-060 — Runner estable + contrato de release (D0):** caracterizar la compatibilidad del `pipeline.kts` de release con un `pipelinek` anterior, *instalado*, certificado y fijado por versión + SHA-256. Separar `runnerVersion` de `targetVersion`; crear un manifiesto/procedencia de artefacto y una verificación externa que rechace tag movido, fuente divergente y ZIP de otro SHA. No usar la candidata para aprobarse a sí misma. Preservar el bootstrap mínimo independiente del DSL.
- **WU-RP-061 — Tag → gate → GitHub Release (D1):** un tag protegido `vX.Y.Z` inicia la evaluación del SHA exacto; Actions prepara runner estable y ejecuta `ci/release.pipeline.kts` (comportamiento de CI en Kotlin), y comprueba los resultados desde fuera del motor. SOLO tras todos los gates de `CERTIFICATION_PROTOCOL.md` sobre el mismo SHA/ZIP se crea la GitHub Release con ZIP inmutable, SHA-256, SBOM, manifiesto y notas. No reutilizar la ruta V1 en cuarentena ni `whenCondition` no certificado para proteger publicación; permisos de publicación/secretos solo tras gate. Reintentos no mueven tags ni reconstruyen un ZIP ya publicado.
- **WU-RP-062 — mise sin nuevo registry obligatorio (D2):** instalar directamente el ZIP de GitHub Releases con el backend de GitHub de mise, fijando versión y patrón del asset real, extracción y Java 21; UAT de CLI y pipeline real en instalación limpia. Alias/registro central opcional posterior, nunca bloqueante para el canal directo. mise descubre las nuevas releases: no publicar por API ni recompilar para este canal.
- **WU-RP-063 — asdf externo mínimo (D3):** crear el plugin independiente `Rubentxu/asdf-pipelinek` con `list-all`, `download`, `install` y, si resulta necesario, `latest-stable`; descargar/verificar el mismo ZIP/SHA, conservar versión/arquitectura y probarlo con asdf real. El índice comunitario es opcional; la instalación por URL del plugin es el primer contrato. Ningún nuevo Step core ni cambio del DSL.
- **WU-RP-064 — SDKMAN canal condicionado (D4):** reutilizar `sdkman-publish.sh`, `sdkman-install-uat.sh` y el workflow existente; corregir sus fixtures Kotlin, carga de `sdk` en la shell hija, validación vinculante de disponibilidad y procedencia SHA/tag. Desacoplar la activación prematura del workflow `release:published` mientras no haya onboarding. Con candidato/credenciales: publicar versión sin default → instalar desde catálogo oficial en runner limpio → validar → promocionar a default; sin alta: `BLOCKED_EXTERNAL`, no PASS falso ni bloqueo de GitHub/mise/asdf.
- **WU-RP-065 — Distribución multicanal y recuperación (D5):** `release:published` dispara solo el fan-out hacia canales sobre el ZIP ya publicado, con resultados y reintentos por canal (`PUBLISHED`, `VERIFIED`, `BLOCKED_EXTERNAL`, `FAILED`) y sin reconstrucción. Verificación externa independiente, recibos por SHA/ZIP/canal, pruebas de repetición, fallo parcial, colisión de tags y secretos. GH ZIP_READY, MISE_READY, ASDF_READY y SDKMAN_READY son certificaciones independientes; nunca promocionar default por un `publish` aparentemente verde.

**Salida del bloque de distribución:** tag nuevo validado → `pipelinek` estable instalado ejecuta el pipeline de release → artefacto único y certificado en GitHub → instalación real vía mise y asdf → SDKMAN cuando onboarding lo permita → recibos por canal con fallo parcial recuperable. El bloque NO altera RP-5 retroactivamente; NO implementa controlador remoto/Jenkins (RP-8/RP-9), NO integra todavía el paquete de configuración YAML (RP-7+) y NO convierte YAML en lenguaje de comportamiento.
