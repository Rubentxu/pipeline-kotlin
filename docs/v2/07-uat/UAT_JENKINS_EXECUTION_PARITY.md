...
## Supersession map (legacy anchors → JEP scenarios)

Binding per cycle `em-0-execution-model-contract-freeze`; bidirectional —
each JEP row carries its predecessor anchor.

| Legacy anchor | Successor | Note |
|---|---|---|
| UAT-DUR-001..007 (block-level `script {}` replay) | JEP-024..028 | scripted replay under DURABLE_KOTLIN_EXECUTION |
| UAT-REC-002 (durable shell kill/recover) | JEP-008..010 | terminal/snapshot split (EM-1) |
| UAT-INC-039 (shell failure provenance) | JEP-008..010 canon rows | RECOVERY_DURABILITY §8 |
| UAT-INC-040 (typed failure semantics) | JEP-008..010 canon rows | RECOVERY_DURABILITY §8 |

## Negative scenarios (N1..N6, widened SPIKE-016 mirror)

| ID | Scenario | Anchor |
|---|---|---|
| N1 | source-digest mismatch on non-terminal entry fails closed | SPIKE-016-N1 |
| N2 | fingerprint collision under adversarial names fails closed; raw concatenation forbidden (ADR-0066) | SPIKE-016-N2 |
| N3 | schema-version downgrade refused by reader (ADR-0067) | SPIKE-016-N3 |
| N4 | INTERRUPTED ordering is terminal and replays typed without relaunch (ADR-0068) | SPIKE-016-N4 |
| N5 | forked structured-concurrency cancellation propagation | SPIKE-016-N5 |
| N6 | worker crash during FAILED_TIMEOUT window keeps TIMEOUT terminal (ADR-0068/ADR-0047) | SPIKE-016-N6 |
