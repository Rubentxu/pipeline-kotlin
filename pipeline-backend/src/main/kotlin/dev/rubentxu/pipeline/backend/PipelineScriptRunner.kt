...
import dev.rubentxu.pipeline.core.jobs.JobLauncherImpl
...
class PipelineScriptRunner {
    ...
    companion object {
        ...
        fun evalWithScriptEngineManager(
            ...
            logger.info("PipelineScriptRunner", "Pipeline executable: ${pipelineExecutable.absolutePath}")
            logger.info("PipelineScriptRunner","Pipeline executable exists: ${pipelineExecutable.exists()}")

            logger.info("PipelineScriptRunner","JAR location: ${jarLocation.absolutePath}")
            val resolveExecutablePath =
                if (pipelineExecutable.exists()) pipelineExecutable.absolutePath else jarLocation.absolutePath
            logger.info("PipelineScriptRunner","Resolve executable path: $resolveExecutablePath")

            return try {
                val pipelineDef = evaluateScriptFile(scriptPath)
                logger.system("PipelineScriptRunner", "Pipeline definition: $pipelineDef")
                val pipeline = buildPipeline(pipelineDef)
                logger.system("PipelineScriptRunner", "Build Pipeline: $pipeline")
                executePipeline(pipeline, scriptPath, configPath, resolveExecutablePath, logger)

            } catch (e: Exception) {
                handleScriptExecutionException(e)
                JobResult(Status.Failure, emptyList(), EnvVars(mapOf()))
            }
        }
    }
}
...
//    val configuration = readConfigFile(normalizeAndAbsolutePath(configPath).toString())
    ...
    if (configurationResult.isFailure) {
        logger.error("PipelineScriptRunner", "Error reading config file: ${configurationResult.exceptionOrNull()?.message}")
    ...
    logger.system("PipelineScriptRunner","Env isAgent: $isAgentEnv")
