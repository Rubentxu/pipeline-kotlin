...
open class CanonicalRuntimeCapabilityAccess(
    ...
    protected fun buildProvided(context: CanonicalRuntimeContext): Map<StepCapability, Any> {
        ...
        // WU-RP-053-MERGE M1 (cut5 forward-port): the adapter ALSO receives the
        // effective cwd derived from `context.shOptions.workingDirectory` (the SAME
        // value `core.sh` consumes in `ShExecution.effectiveOptions`); when present
        // it is composed with the stage workspace via the adapter's single
        // `effectiveRoot()` seam so `dir(...) { writeFile / readFile / fileExists }`
        // resolves paths against the block cwd. When null the adapter falls back
        // to the per-stage workspace or the project-workspace override, preserving
        // byte-equivalent behaviour for stages that never enter a `dir(...)` block.
        ...
        //
        // WU-RP-053-MERGE M1 (cut5 forward-port): when the runtime context carries an
        // effective cwd (`context.shOptions.workingDirectory` — set by the body loop
        // on `dir(...)`/`withEnv`/etc.), it wins over the bare workspace root so
