...
class CoreCleanWsStepContractSuiteTest {
    ...
    @Test
    fun `G5 LEGACY_REMOVED invariant post-WU-LPR-301-G5 — counters converge to 0 0 0 (burn-down closed)`() {
        assertFalse(
            "core.cleanWs" in CanonicalCoreStepCommand.LEGACY_PLUGIN_IDS,
            "core.cleanWs MUST NOT be in LEGACY_PLUGIN_IDS post-G5 (LEGACY_REMOVED)",
        )
        assertEquals(
            ...
            "the residual MUST converge to the empty set post-WU-LPR-301/G5 — every legacy key retired",
        )
        assertFalse(
            "core.cleanWs" in CanonicalCoreStepMetadata.pluginIds,
        ...
        assertEquals(
            ...
            "the metadata table MUST also converge to the empty set post-WU-LPR-301/G5",
        )
        assertFalse(
        ...
        assertFalse(
            "core.load" in CanonicalCoreStepMetadata.pluginIds,
            "core.load metadata row MUST be physically removed at WU-LPR-301/G5",
        )
        assertFalse(
            "core.waitUntil" in CanonicalCoreStepMetadata.pluginIds,
            "core.waitUntil metadata row MUST be physically removed at WU-LPR-301/G5",
        )
    }
}
