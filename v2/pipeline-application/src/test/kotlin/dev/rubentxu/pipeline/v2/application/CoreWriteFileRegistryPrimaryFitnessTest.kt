...
class CoreWriteFileRegistryPrimaryFitnessTest {
    ...
    @Test
    fun `G5 LEGACY_REMOVED post-WU-LPR-301-G5 - core dot writeFile stays absent (closed set)`() {
        assertTrue(
            key.value !in CanonicalCoreStepCommand.LEGACY_PLUGIN_IDS,
            "core.file.writeFile MUST remain routed via the registry post-WU-LPR-301/G5 LEGACY_REMOVED",
        )
        assertEquals(
            0,
            CanonicalCoreStepCommand.LEGACY_PLUGIN_IDS.size,
