...
/**
 * WU-RP-053 cut5 CLI integration tests. Re-enabled in WU-RP-053-FOLLOWUP via the
 * `--workspace-mode=project` opt-in flag. Each test invokes the installed binary with
 * `--workspace-mode=project` so the CLI uses script's parent directory as the workspace
 * (Jenkins-parity, cut5 PROJECT mode), without changing the legacy default for existing UATs.
 */
...
class WURp053WorkspaceCliTest {
    ...
    fun `bare and nested relative scripts use their own directory for canonical durable workspace`() {
        ...
        listOf("bare.pipeline.kts", "nested/nested.pipeline.kts").forEachIndexed { index, name ->
            val (exit, output) = invoke(
                project, "run",
                "--db", state.resolve("$index.sqlite").toString(),
    ...
    fun `explicit workspace wins for canonical and scripted durable bare filename`() {
        ...
        val (scriptedExit, scriptedOutput) = invoke(project, "run", "--db",
            state.resolve("scripted.sqlite").toString(), "--workspace", workspace.toString(),
            "--workspace-mode", "project",
            "scripted.pipeline.kts")
