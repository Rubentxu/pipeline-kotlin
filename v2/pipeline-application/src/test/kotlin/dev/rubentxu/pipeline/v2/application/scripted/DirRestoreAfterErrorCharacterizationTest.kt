...
class DirRestoreAfterErrorCharacterizationTest {
    ...
    @org.junit.jupiter.api.Disabled(
        "Depends on WU-RP-053 cut5 workspace authorization (ShOptions.effectiveWorkingDirectory) " +
            "which is rebased on wu/rp-053-cut5-stash-cwd-rebase but NOT on this branch " +
            "(wu/rp-053-dir-failure-mode). Re-enable when the workspace-authorization slice " +
            "is merged onto the same branch as the Contained-failure slice. See PR #96 / " +
            "branch wu/rp-053-cut5-stash-cwd-rebase (commit aebd6207).",
    )
