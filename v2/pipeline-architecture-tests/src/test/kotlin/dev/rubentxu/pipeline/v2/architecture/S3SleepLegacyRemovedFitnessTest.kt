...
class S3SleepLegacyRemovedFitnessTest {
    ...
    @Test fun `three residual legacy authorities converge to exact ten step snapshots`() {
        ...
        val actualDispatchers = Files.list(durable).use { paths -> paths.map { it.fileName.toString() }
            ...
            .toList().toSet() }
        assertEquals(setOf(
            "CanonicalWriteFileNodeDispatcher.kt", "CanonicalEmitEventNodeDispatcher.kt",
        ...
        assertEquals(10, legacyIds().size)
        assertEquals(10, metadataKeys.size)
        assertEquals(10, actualDispatchers.size)
    }
}
