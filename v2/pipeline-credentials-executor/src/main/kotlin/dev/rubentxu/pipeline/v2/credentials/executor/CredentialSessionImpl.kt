...
class CredentialSessionImpl(
     ...
     * Emits [CredentialUnbound] for all resolved credentials before cleanup.
     * This is the SINGLE cleanup owner - public callers MUST NOT independently
     * emit CredentialUnbound events.
     *
     * Cleanup order:
     * 1. Emit CredentialUnbound for all resolved credentials
     * 2. Wipe materializer paths (files created for file-based credentials)
     * 3. Wipe all active handles (secret bytes filled with zeros)
     */
    override fun close() {
        if (_closed) return
        _closed = true
        ...
        // Emit CredentialUnbound for all resolved credentials
        for (resolved in _resolved) {
            try {
                emitCredentialUnbound(resolved.credentialsId)
            } catch (t: Throwable) {
