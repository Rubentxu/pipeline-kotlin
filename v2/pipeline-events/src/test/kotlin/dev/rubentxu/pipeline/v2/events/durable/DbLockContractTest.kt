...
class DbLockContractTest {
    ...
    fun `single instance serial appends`() {
        ...
        val eventStore = dev.rubentxu.pipeline.v2.events.SqliteEventStore(dbPath)
        ...
        journal.append(
            dev.rubentxu.pipeline.v2.domain.durable.RerunOperation(
                id = "op-single-1",
                fingerprint = dev.rubentxu.pipeline.v2.domain.durable.Fingerprint("a".repeat(64)),
                input = dev.rubentxu.pipeline.v2.domain.durable.OperationInput("step", mapOf(), "run-1", 1),
                output = null,
    ...
    fun `two instances parallel threads serialize`() {
        ...
        val eventStore = dev.rubentxu.pipeline.v2.events.SqliteEventStore(dbPath)
        val clock = object : dev.rubentxu.pipeline.v2.domain.durable.Clock {
            override fun now() = java.time.Clock.systemUTC().instant()
        }
            ...
            try {
                for (i in 1..10) {
                    journal2.append(
                        dev.rubentxu.pipeline.v2.domain.durable.RerunOperation(
                            id = "op-parallel-2-$i",
                            fingerprint = dev.rubentxu.pipeline.v2.domain.durable.Fingerprint("b".repeat(64)),
                            input = dev.rubentxu.pipeline.v2.domain.durable.OperationInput("step", mapOf(), "run-2", 1),
                            output = null,
                            status = dev.rubentxu.pipeline.v2.domain.durable.OperationStatus.SUCCEEDED,
                            attempt = 1,
                        )
    ...
    fun `busy_timeout pragma set on every connection`() {
        ...
        val factory = dev.rubentxu.pipeline.v2.events.SqliteEventStore(dbPath)
